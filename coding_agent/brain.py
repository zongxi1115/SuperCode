            ...
            [
                ...
                "\n".join(
                    [
                        ...
                        'JSON 格式：{"action":"tool 或 final","thought":"当前思路","tool_name":"工具名","tool_arguments":{},"final_answer":"最终答复"}',
                        "规则：",
                        "1. 如果 action 是 tool，必须提供 tool_name 和 tool_arguments。",
                        "2. 如果 action 是 final，必须提供 final_answer。",
                        "3. 在真正修改文件前，优先先探索相关目录、文件和引用关系。",
                        "4. 普通答疑可以直接 final；需要查看或修改项目时再调用工具。",
                        "5. 命令执行工具优先使用 `excecute`；如果输出里提到 `execute`，可视为同义工具。",
                    ]
                ),
            ]
        )
