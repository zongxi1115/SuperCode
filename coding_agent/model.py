    ...
    def _build_system_prompt(
        ...
        if response_mode == "native_tools":
            protocol_lines = [
                ...
                "8. 如果 execute/excecute 返回的结果里 `status` 是 `running` 且 `awaiting_input` 为 false，说明命令仍在后台执行且本次等待窗口已到期，应按需要调用 `terminal_wait` 继续等待。`exit_reason=timeout` 表示本次 timeout 内没有等到完成，不代表命令失败。多个活动终端并存时，必须显式传 `terminal_id`。",
            ...
            protocol_lines = [
                ...
                "9. 如果 execute/excecute 返回的结果里 `status` 是 `running` 且 `awaiting_input` 为 false，说明命令仍在后台执行且本次等待窗口已到期，应按需要调用 `terminal_wait` 继续等待。`exit_reason=timeout` 表示本次 timeout 内没有等到完成，不代表命令失败。多个活动终端并存时，必须显式传 `terminal_id`。",
