...
ALLOWED_AGENT_TYPES = {"coding", "deploy", "plan"}
...
def _build_route_messages(user_message: str, context: dict[str, Any]) -> list[dict[str, object]]:
    ...
    system_prompt = (
        "你是 SuperCode 的轻量路由器。只输出一个词：coding、plan 或 deploy。\n"
        "plan=澄清需求/调研/实施计划；coding=改代码/调试/测试/解释代码；deploy=部署/上传/服务器/环境变量/发布/回滚/远程连接。\n"
