...
def _format_plan_answer_batches(answer_batches: list[dict[str, Any]]) -> str:
    blocks: list[str] = []
    for batch in answer_batches:
        ...
        if not isinstance(answers, list) or not answers:
            continue
        blocks.append(_format_plan_answer_summary(title, answers))
    return "\n\n".join(blocks)
...
def _sync_plan_answers_from_history(session: Any) -> None:
    ...
    if state.get("answered_questions"):
        state["answer_summary"] = _format_plan_answer_batches(state["answered_questions"])
    ...
    state["answer_summary"] = _format_plan_answer_batches(batches)
